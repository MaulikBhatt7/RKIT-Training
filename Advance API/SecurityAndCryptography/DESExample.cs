...
{
    /// <summary>
    /// A class that demonstrates DES encryption and decryption.
    /// </summary>
    public class DESExample
    {
        /// <summary>
        /// Encrypts the given plaintext using DES encryption with the specified key and IV.
        /// </summary>
        /// <param name="plaintext">The plaintext to encrypt.</param>
        /// <param name="key">The encryption key (must be 8 bytes long).</param>
        /// <param name="iv">The initialization vector (must be 8 bytes long).</param>
        ...
        public string Encrypt(string plaintext, string key, string iv)
        {
            ...
            // Create a DES object for encryption.
            using (DES objDES = DES.Create())
            {
                objDES.Key = keyBytes;  // Set the encryption key.
                objDES.IV = ivBytes;    // Set the initialization vector.
                objDES.Padding = PaddingMode.PKCS7; // Use PKCS7 padding.

                // Create an encryptor object.
                using (ICryptoTransform objEncryptor = objDES.CreateEncryptor(objDES.Key, objDES.IV))
                {
        ...
        /// <summary>
        /// Decrypts the given ciphertext using DES decryption with the specified key and IV.
        /// </summary>
        /// <param name="ciphertext">The ciphertext to decrypt (Base64-encoded).</param>
        /// <param name="key">The decryption key (must be 8 bytes long).</param>
        /// <param name="iv">The initialization vector (must be 8 bytes long).</param>
        ...
        public string Decrypt(string ciphertext, string key, string iv)
        {
            ...
            using (DES objDES = DES.Create())
            {
                objDES.Key = keyBytes;  // Set the decryption key.
                objDES.IV = ivBytes;    // Set the initialization vector.
                objDES.Padding = PaddingMode.PKCS7; // Use PKCS7 padding.

                // Create a decryptor object.
                using (ICryptoTransform objDecryptor = objDES.CreateDecryptor(objDES.Key, objDES.IV))
                {
